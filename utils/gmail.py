...
class Gmail:
    def __init__(
        self, user_id="me", alerts_label=["Academia"], data_path="./", digest_address=""
    ):
        # set user_id
    ...
    def fetch_labeled_msg_ids(self):
        ...
        try:
            response = (
                self.service.users()
                .messages()
                .list(userId=self.user_id, labelIds=label_ids)
                .execute()
            )
            message_ids = []
            ...
            while "nextPageToken" in response:
                ...
                response = (
                    self.service.users()
                    .messages()
                    .list(userId=self.user_id, labelIds=label_ids, pageToken=page_token)
                    .execute()
                )
                message_ids.extend(response["messages"])
            return message_ids
        ...
        for msg_id in msg_ids:
            try:
                message = (
                    self.service.users()
                    .messages()
                    .get(userId=self.user_id, id=msg_id["id"])
                    .execute()
                )
                msgs.append(message)
            except Exception as e:
    ...
    def mark_msg_read(self, msg_ids):
        ...
        for msg in msg_ids:
            self.service.users().messages().modify(
                userId=self.user_id, id=msg["id"], body=body
            ).execute()

    def delete_msg(self, msg_ids):
        for msg in msg_ids:
            try:
    ...
    def send_digest(self, formatted_message, len_entries):
        ...
        try:
            message = self.create_message(
                "me",
                self.digest_address,
                f"Scholar Alerts Digest({len_entries}) {time.strftime('%Y-%m-%d')}",
                formatted_message,
            )
            send_resp = (
                self.service.users()
                .messages()
                .send(userId=self.digest_address, body=message)
                .execute()
            )
            return send_resp["id"]
        except Exception as e:
